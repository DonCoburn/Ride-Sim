            ...
            if len(activities) >= 2:
                ...
                while counter < len(activities):
                    total_distance += manhattan_distance(
                        activities[counter - 1].location,
                        activities[counter].location)
                    counter += 1
            count += 1
    ...
    def _average_ride_distance(self) -> float:
        ...
        for activities in self._activities[DRIVER].values():
            if len(activities) >= 2:
                ...
                while counter < len(activities):
                    ride_distance += \
                        _average_ride_distance_helper(activities[counter - 1],
                                                      activities[counter])
                    counter += 1
            count += 1
...
# Helpers
def _average_ride_distance_helper(activity1: Activity,
                                  activity2: Activity) -> float:
    """Helper to calculate average ride distance."""
    if activity2.description == DROPOFF:
        return manhattan_distance(activity1.location, activity2.location)
    return 0

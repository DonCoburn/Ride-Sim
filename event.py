...
class Pickup(Event):
    ...
    def __init__(self, timestamp: int, rider: Rider, driver: Driver) -> None:
        """Initialize a PickupRequest event.
        """
        super().__init__(timestamp)
        self.rider = rider
        self.driver = driver
    ...
    def __str__(self) -> str:
        """Return a string representation of this event."""
        return "Driver {} --> Rider {}".format(self.driver, self.rider)
    ...
    def do(self, dispatcher: Dispatcher, monitor: Monitor) -> List[Event]:
        ...
        if self.rider.status == WAITING:
            ...
            monitor.notify(self.timestamp, RIDER, PICKUP, self.rider.identifier,
                           self.rider.origin)  # gives me the 0.5 :)
            drop_off_time = self.driver.start_ride(self.rider)
            event = Dropoff(self.timestamp + drop_off_time, self.rider,
                            self.driver)
            events.append(event)
...
class Dropoff(Event):
    ...
    def __str__(self) -> str:
        """Return a string representation of this event."""
        return "{} --- Driver {} dropped off Rider {}".format(self.timestamp,
                                                              self.driver,
                                                              self.rider)
        ...
        for line in file:
            ...
            if event_type == "DriverRequest":
                ...
                driver = Driver(tokens[2], deserialize_location(tokens[3]),
                                int(tokens[4]))
                driver.set_time_stamp(timestamp)
                ...
                rider.set_time_stamp(timestamp)
